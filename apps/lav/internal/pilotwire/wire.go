...
// (internal/pilotrunner, the server) that keeps a character's process alive
// across a daemon restart. The two only ever talk over a filesystem-local
// Unix domain socket — never a network port — so this package adds no new
...
// pilotDir is where every character's socket, durable transcript and
...
// a character id. Its mere existence and dial-ability is how the daemon
// tells a still-running detached process apart from one that already
// exited — the one signal presence is ever computed from.
func SocketPath(lavHome, id string) string {
	return filepath.Join(pilotDir(lavHome), id+".sock")
}

// TranscriptPath is where a character's raw provider stdout lines are
// appended durably, one per line, independent of the daemon process's own
// lifetime.
...
// StderrPath is where a character's process's stderr is captured, kept
...
// ClientMsg is sent by the daemon over a pilot-runner's control socket:
// "attach" (subscribe, replaying everything after Since), "stdin" (relay
// Data to the child process's stdin), or "kill" (send SIGKILL to the child —
// Stop/Interrupt-via-kill for Cursor).
...
// ServerMsg is sent from a pilot-runner to the daemon's attached connection:
// one transcript line (Seq > 0), or a terminal exit notice.
...
// recorded (a fresh character, or one whose offset file was lost).
...
// KnownFiles lists every per-character file under the pilot directory
// (socket, transcript, offset, stderr log), keyed by the character id its
// name starts with. Used by the daemon's startup orphan sweep to find files
// with no character behind them any more.
func KnownFiles(lavHome string) (map[string][]string, error) {
	entries, err := os.ReadDir(pilotDir(lavHome))
	if err != nil {
	...
	for _, e := range entries {
		...
		name := e.Name()
		for _, suffix := range []string{".sock", ".jsonl", ".offset", ".stderr.log"} {
			if len(name) > len(suffix) && name[len(name)-len(suffix):] == suffix {
				id := name[:len(name)-len(suffix)]
				byID[id] = append(byID[id], filepath.Join(pilotDir(lavHome), name))
				break
			}
		}
	}
